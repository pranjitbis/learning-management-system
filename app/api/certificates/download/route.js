...
export async function GET(request) {
  try {
    ...
    if (!authHeader?.startsWith("Bearer ")) {
      return Response.json({ error: "Unauthorized - Missing or invalid token" }, { 
        status: 401,
    ...
    try {
      ...
      return Response.json({ error: "Unauthorized - Invalid token" }, { 
        status: 401,
        headers: { 'Content-Type': 'application/json' }
      });
    }
    ...
    const courses = user.access.map(access => {
      ...
      const completedVideos = Object.values(videoProgressMap)
        .filter(vp => vp.completed).length;
    ...
    const inProgressCourses = courses.filter(c => !c.isCompleted && c.completedVideos > 0);
    ...
    const responseData = {
      ...
      stats: {
        ...
        totalVideosWatched: user.videoProgress.filter(vp => vp.progress > 0).length,
